...
> Team Kopiko | K.J. Somaiya College of Engineering
...
🔗 **Live App:** [Add your Streamlit Cloud URL after deploy](#deployment-streamlit-community-cloud)
...
One-click public demo for judges — no Docker required.

### 1. Push to GitHub

Commit the repo **without** secrets or virtualenv:

- Do **not** commit: `.venv/`, `.env`, `.streamlit/secrets.toml`
- **Optional:** commit `data/*.csv` and `data/*.json` for a faster cold start (otherwise the app generates them on first load)

### 2. Create the app

1. Open [share.streamlit.io](https://share.streamlit.io) and sign in with GitHub.
2. Click **New app** → select your `zombieshield` repository.
...
Streamlit installs Python packages from `requirements.txt`. The repo includes an empty `packages.txt` (no extra Linux/apt packages needed for scikit-learn wheels).

### 3. Optional: Claude API key (secrets)
...
Or use the in-app sidebar field (session only). The app works fully **without** this key.

### 4. First load

On the first visit, the app may take **30–60 seconds** while it generates synthetic data and runs classification + ML. Later loads use cached outputs (`@st.cache_data`, 60s TTL). Use **Re-run Full Scan** in the sidebar to refresh.

### 5. Smoke test after deploy

- [ ] Overview charts and KPIs load  
- [ ] API Registry table loads  
- [ ] Risk Drill-Down → Generate Remediation Playbook  
- [ ] ML Insights tab shows accuracy metrics  
- [ ] Compliance checklist renders  

### 6. Update README live link

Paste your public URL into the **Live App** line at the top of this README.
...
| `dashboard/app.py` | Main dashboard UI |
| `requirements.txt` | Python dependencies (pip) |
| `packages.txt` | System/apt dependencies (empty — none required) |
| `.streamlit/config.toml` | Theme and server options |
| `.streamlit/secrets.toml.example` | Template for Claude API key on Cloud |
